...
- Kolej składa się z **72 czteroosobowych krzesełek**. Jednocześnie może być zajętych **36 krzesełek** (druga połowa wraca pusta w dół).
- Na krzesełku mogą siedzieć maksymalnie:
  - **2 rowerzystów**, albo
  - **1 rowerzysta + 2 pieszych**, albo
  - **4 pieszych**.
...
Najwygodniej uruchomić wersję „demo” w tmux (nie wymaga GUI):
...
- **wyciąg** – symulacja obiegu 72 krzesełek: połowa trasy w górę z pasażerami (max 36 zajętych krzesełek), połowa w dół pusta.
...
Poniżej znajduje się seria **4 testów** (skrypty w katalogu `tests/`), których celem jest sprawdzenie działania symulacji w warunkach “brzegowych” oraz potwierdzenie, że nie dochodzi do:
...
### Test 1 – Limity krzesełek i zasady sadzania (72/36 + 4 sloty)
...
1. Linia startowa wyciągu: `RZEDOW=72` oraz `SLOTY/RZAD=4` → potwierdza parametry modelu.
